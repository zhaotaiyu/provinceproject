...
class QinghaiSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        total_page = int(
            re.findall(".*?pc:(\d+).*", response.xpath("//a[@sf='pagebar']/@*[name()='sf:data']").extract_first())[
                0])
    ...
    def parse_company(self, response):
        if response.meta.get("mark") == "BS":
            ...
            if qinghai["social_credit_code"] is None:
                qinghai["social_credit_code"] = response.xpath(
                    "//div[@class='bottom']/dl[1]/dd/text()").extract_first()
            ...
            qinghai["build_date"] = "-".join(
                re.findall("\d+", str(response.xpath("//div[@class='bottom']/dl[3]/dt/text()").extract_first())))
            ...
            aptitude_url = "http://139.170.150.135/dataservice/query/comp/caDetailList/{}".format(
                province_company_id)
            yield Request(aptitude_url, callback=self.parse_companyaptitude,
                          meta={"province_company_id": province_company_id,
                                "company_name": qinghai["company_name"]})
        else:
            beian = BeianItem()
