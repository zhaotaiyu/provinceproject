...
class QinghaiSpider(scrapy.Spider):
    ...
    def parse_company(self,response):
        if response.meta.get("mark") == "BS":
            ...
            beian = BeianItem()
            beian["corpcode"] = response.xpath("//div[@class='bottom']/dl[1]/dt/text()").extract_first()
            beian["corpname"] = response.xpath("//span[@class='user-name']/text()").extract_first()
            beian["legalman"] = response.xpath("//div[@class='bottom']/dl[2]/dd/text()").extract_first()
